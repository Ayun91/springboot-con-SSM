...
<script>
    ...
    layui.use(['table', 'form', 'upload', 'jquery'], function () {
        ...
        table.render({
            elem: '#demo'
            , height: 550
            , url: '${pageContext.request.contextPath}/viewTestResultScl/getReturn.do'
            , dataType: 'json'
            , page: true //开启分页
            , limit: 10
                ...
                case "delete":
                    if (data.length === 0) {
                        ...
                        layer.confirm('真的删除行么', function (index) {
                            $.ajax({
                                url: "${pageContext.request.contextPath}/viewTestResultScl/deleteReturn.do",
                                data: {id: ids.join(",")},
                                //响应的数据类型
                                dataType: "JSON",
                                type: "POST"
        ...
        form.on('submit(updateUserFilter)', function (data) {
            //提交表单数据
            $.ajax({
                url: "${pageContext.request.contextPath}/viewTestResultScl/updateReturn.do"
                , data: data.field
                , dataType: "json"
                , type: "POST"
                , success: function (data) {
