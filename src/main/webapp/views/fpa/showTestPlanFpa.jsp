...
<div id="app">
    ...
    <form class="layui-form" action="" id="addUserForm" style="display: none" enctype="multipart/form-data">
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn checkAdd" lay-submit lay-filter="add">立即提交</button>
    ...
    <div class="layui-form-item" id="viewURL" style="display: none;margin-top: 30px">
        ...
        <div class="layui-input-block">
            <input type="text" id="copyText" class="layui-input" readonly
...
<script>
    ...
    layui.use(['table', 'form', 'jquery', 'laydate'], function () {
        ...
        table.render({
            elem: '#demo'
            , height: 541
            , url: '${pageContext.request.contextPath}/testPlanFpa/getAll.do' //数据接口
        ...
        form.on('submit(add)', function (data) {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/testPlanFpa/save.do",
                type: "POST",
                data: data.field,
                ...
                case 'delete':
                    if (data.length === 0) {
                        ...
                        layer.confirm('真的删除行么', function (index) {
                            $.ajax({
                                url: "${pageContext.request.contextPath}/testPlanFpa/delete.do?ids=" + ids,
                                type: "DELETE",
                                success: function (ret) {
                                    if (ret.code === 0) {
                                        layer.closeAll();//关闭对话框
        ...
        table.on('tool(test)', function (obj) { // 注：test 是 table 原始标签的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'ewm') {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    $.ajax({
                        url: "${pageContext.request.contextPath}/testPlanFpa/delete.do?ids=" + data.id,
                        type: "DELETE",
                        // data: {id: data.userid},
                        dataType: "JSON",
                        success: function (ret) {
        ...
        $(".checkName").blur(function () {
            $.ajax({
                url: "${pageContext.request.contextPath}/testPlanFpa/repetitionName.do",
                data: {testName: $(".checkName").val()},
                dataType: "json",
                type: "get",
                success: function (re) {
                    if (re.code === 0) {
