...
<div id="app">
    ...
    <form class="layui-form" action="" id="addUserForm" style="display: none" enctype="multipart/form-data">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <select name="type" lay-verify="required">
                    <option value="1">前十五道</option>
                    <option value="2">后十五道</option>
                </select>
            </div>
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn checkAdd" lay-submit lay-filter="addQuestion">立即提交</button>
    ...
    <form class="layui-form" action="" id="updateForm" lay-filter="updateForm" style="display: none">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="question" required lay-verify="required"
                       placeholder="请输入问题" autocomplete="off"
                       class="layui-input"/><span class="msgSpan" style="color: #eb7350"></span>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">选项A</label>
...
<script>
    ...
    layui.use(['table', 'form', 'jquery', 'util', 'upload'], function () {
        ...
        table.render({
            elem: '#demo'
            , height: 'full-20'
            , url: '${pageContext.request.contextPath}/questionFpa/getAll.do' //数据接口
            , page: true //开启分页
            , limit: 9
            ...
            , cols: [[ //表头
                ...
                , {field: 'type', title: '类型', width: 90, sort: true}
        ...
        form.on('submit(addQuestion)', function (data) {
            $.ajax({
                url: "${pageContext.request.contextPath}/questionFpa/save.do",
                type: "POST",
        ...
        form.on('submit(update)', function (data) {
            $.ajax({
                url: "${pageContext.request.contextPath}/questionFpa/updateQuestionFpa.do",
                type: "PUT",
                data: JSON.stringify(data.field),
                // headers: {'content-type': 'application/json'},
                contentType: 'application/json;charset=utf-8',
        ...
        table.on('tool(test)', function (obj) { // 注：test 是 table 原始标签的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'update') {
                ...
                layer.confirm('确定删除吗？', function (index) {
                    $.ajax({
                        url: "${pageContext.request.contextPath}/questionFpa/delete.do?ids=" + data.id,
                        type: "DELETE",
                        // data: {id: data.userid},
                        dataType: "JSON",
                        success: function (ret) {
        ...
        $(".checkName").blur(function () {
            $.ajax({
                url: "${pageContext.request.contextPath}/questionFpa/repetitionName.do",
                data: {question: $(".checkName").val()},
                dataType: "json",
                type: "get",
                success: function (re) {
                    if (re.code === 0) {
