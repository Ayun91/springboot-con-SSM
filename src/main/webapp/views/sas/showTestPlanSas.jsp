...
<div id="app">
    ...
    <form class="layui-form" action="" id="addUserForm" style="display: none" enctype="multipart/form-data">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="testName" required lay-verify="required"
                       placeholder="请输入测试名称" autocomplete="off"
                       class="layui-input" id="planAdd"><span class="tips" style="color: red"></span>
            </div>
        </div>
        <div class="layui-form-item">
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn" lay-submit lay-filter="add" id="addPlan">立即提交</button>
    ...
    <form class="layui-form" action="" id="updateForm" lay-filter="updateForm" style="display: none">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="testName" required lay-verify="required"
                       placeholder="请输入测试名称" autocomplete="off"
                       class="layui-input" id="planUp"><span class="tips" style="color: red"></span>
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label">结束时间</label>
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn" lay-submit lay-filter="update" id="upPlan">立即提交</button>
                <button type="reset" class="layui-btn layui-btn-primary">重置</button>
            </div>
        </div>
    </form>
...
<script>
    ...
    layui.use(['table', 'form', 'jquery', 'laydate'], function () {
        ...
        table.render({
            elem: '#demo'
            , height: 541
            , url: '${pageContext.request.contextPath}/tblTestPlanSas/getAll.do' //数据接口
        ...
        form.on('submit(add)', function (data) {
            ...
            $.ajax({
                url: "${pageContext.request.contextPath}/tblTestPlanSas/save.do",
                type: "POST",
                data: data.field,
                dataType: "JSON",
        ...
        form.on('submit(update)', function (data) {
            $.ajax({
                url: "${pageContext.request.contextPath}/tblTestPlanSas/update.do",
                type: "PUT",
        ...
        table.on('toolbar(test)', function (obj) {
            ...
            switch (obj.event) {
                case 'add':
                    $("#planAdd").val("");
                    $(".tips").html("");
                    $('#addPlan').removeClass("layui-btn-disabled").attr("disabled", false);
                    layer.open({
                        type: 1,
                ...
                case 'delete':
                    if (data.length === 0) {
                        ...
                        layer.confirm('真的删除行么', function (index) {
                            $.ajax({
                                url: "${pageContext.request.contextPath}/tblTestPlanSas/delete.do?ids=" + ids,
                                type: "DELETE",
                                success: function (ret) {
                                    ``
                                    if (ret.code == 0) {
        ...
        table.on('tool(test)', function (obj) { // 注：test 是 table 原始标签的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'ewm') {
                ...
                $(".tips").html("");
                $('#upPlan').removeClass("layui-btn-disabled").attr("disabled", false);
                layer.open({
                    type: 1,
                ...
                layer.confirm('确定删除吗？', function (index) {
                    $.ajax({
                        url: "${pageContext.request.contextPath}/tblTestPlanSas/delete.do?ids=" + data.id,
                        type: "DELETE",
                        // data: {id: data.userid},
                        dataType: "JSON",
                        success: function (ret) {
        ...
        * 添加测试计划 唯一性校验
        * */
        $("#planAdd").blur(function () {
            $.ajax({
                url: "${pageContext.request.contextPath}/tblTestPlanSas/repetitionPlanName.do",
                data: {testName: $("#planAdd").val()},
                dataType: "json",
                type: "get",
                success: function (re) {
                    if (re.code == 0) {
                        $("#planAdd").next().html(re.msg);
                        $('#addPlan').removeClass("layui-btn-disabled").attr("disabled", false);
                    } else {
                        $("#planAdd").next().html(re.msg);
                        $('#addPlan').addClass("layui-btn-disabled").attr("disabled", true);
                    }
                }
            })
        })
            ...
            $.ajax({
                ...
                success: function (re) {
                    if (re.code == 0) {
                        ...
                        $("#planUp").next().html(re.msg);
                        $('#upPlan').addClass("layui-btn-disabled").attr("disabled", true);
                    }
                }
            })
        })
